...
class PortalRepositoryImpl implements PortalRepository {
  ...
  Future<ApiResult<PortalInfoEntity>> getPortalInfo() async {
    ...
    if (EnvConfig.useDummy) {
      const dummy = PortalInfoEntity(
        totalMahasiswa: '12.500+',
        totalDosen: '450+',
        totalFakultas: '5',
        totalProdi: '14',
        rektorName: 'Drs. Humuntal Rumapea, M.Kom.',
        rektorSambutan: 'Selamat datang di Universitas Methodist Indonesia (UMI Medan). UMI senantiasa bertekad menyelenggarakan pendidikan tinggi berkualitas prima berlandaskan kasih Kristus dan siap melahirkan lulusan unggul berdaya saing global.',
        visiMisiText: 'Menjadi Universitas Unggul dalam Pengembangan IPTEKS Berlandaskan Kasih Kristus dan Berdaya Saing Global.',
        sejarahText: 'Universitas Methodist Indonesia (UMI) didirikan oleh Gereja Methodist Indonesia (GMI) Wilayah 2. Memiliki dua lokasi kampus strategis di Medan (Kampus I Jl. Hang Tuah & Kampus II Jl. Pasar II Tanjung Sari Selayang).',
        banners: [
          BannerItemEntity(
            id: 'b1',
            tag: 'PMB UMI 2026',
        ...
        news: [
          PortalNewsEntity(
            ...
            content: 'Universitas Methodist Indonesia (UMI) Medan membuka pendaftaran calon mahasiswa baru melalui pmb.methodist.ac.id. Tersedia jalur Beasiswa Prestasi Rapor, jalur Olahraga/Seni, serta diskon registrasi bagi pendaftar awal.',
          ),
          PortalNewsEntity(
            id: 'n2',
            category: 'Pengumuman',
            ...
            content: 'Pengisian Kartu Rencana Studi (KRS) diselenggarakan terpusat melalui aplikasi SIMAK UMI Mobile. Mahasiswa diharapkan melakukan konfirmasi registrasi pembayaran sebelum batas akhir pengisian.',
          ),
          PortalNewsEntity(
            id: 'n3',
            ...
            content: 'Lembaga Penelitian dan Pengabdian Masyarakat (LPPM) UMI mengundang para dosen dan peneliti untuk mengirimkan karya ilmiah pada Jurnal Methoda, Jurnal Methodika, Jurnal MethodAgro, dan Jurnal Methonomi.',
          ),
          PortalNewsEntity(
            id: 'n4',
            ...
            content: 'Prestasi membanggakan diraih oleh sivitas akademika Universitas Methodist Indonesia pada ajang kompetisi karya tulis ilmiah dan inovasi teknologi informasi nasional.',
          ),
        ],
        agendas: [
      ...
      return const ApiSuccess(dummy);
    }

    try {
      final response = await dioClient.dio.get('/public/portal-info');
      if (response.statusCode == 200 && response.data != null) {
        final map = response.data['data'] as Map<String, dynamic>;
        final portalInfo = PortalInfoEntity.fromMap(map);
        return ApiSuccess(portalInfo);
      }
      return const ApiFailure('Format respons API tidak sesuai.');
    } catch (e) {
